...
Die schlechteren Ergebnisse der Odometrie können wie folgt erklärt werden. Mit wachsender Entfernung nehmen auch Fehler durch zum Beispiel unterschiedliche Dr\"ucke in den Reifen oder eine verst\"arkte Reibung oder Unebenheiten auf anderem Gel\"ande zu. Weitere Fehlerquelle sind h\"oheren Geschwindigkeiten und engeren Kurven, dort neigen die R\"ader zum Durchdrehen und Wegrutschen. 	



Von der Lokalisierungsperformance abgesehen gibt es weitere Vor- und Nachteile. So benötigt AMCL eine Karte um funktionieren zu können. Außerdem ist der Rechenaufwand bei AMCL höher und daher ist AMCL seltener vefügbar. 
...
{
	\textbf{TODO Doppelter text, Genauigkeit der Karte, Failure beim Start im Hiwiraum erwähnen ?}
...
F\"ur die Zukunft kann die relative Positionsbestimmung verbessert werden, indem die Reifengr\"o{\ss}en und Achsenl\"ange präzise gemessen wird und diese Information einprogrammiert wird. Außerdem wäre es möglich weitere Sensoren wie Accelerometer, Gyroskop und Magnetometer zu verwenden. 
\\
Da die Fehler von \textit{AMCL} bei der Lokalisierung sofort ersichtlich waren ist es eine Idee neuronale Netze zur Lokalisierung mit Laserdaten zu verwenden. Es wäre auch sehr einfach dafür Trainingsdaten zu gewinnen.

Der Giovanni Controller könnte dahingehend verbessert werden dass er mit variblen Geschwindigkeiten zurecht kommt und so auch engere Kurven fahren kann.

\textbf{TODO dass mit der konstatnen Geschwindigkeit noch beim giovanni test erwähnen}
