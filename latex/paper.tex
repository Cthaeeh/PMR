...
\section{Einleitung}
Autonome Fahrzeuge sind ein großes Thema der Zukunft. Damit diese sich lokalisieren können und einem Pfad folgen braucht es verschiedene Algorithmen.
Zur Lokalisierung kann der AMCL-Algorithmus von XYZ in Paper zx beschrieben werden verwendet werden. Dieser benötigt jedoch eine Karte welche mit dem SLAM-Gmapping Algo von XYZ im Paper xyz beschrieben wird. Für die Pfadverfolgung kann der Algorithmus von giovanni xyz im Paper xyz beschrieben genutzt werden. 

Die drei Algorithmen werden im Abschnitt 1) genauer erläutert. Dann folgt eine kurze Beschreibung unserer Testplattform und dem Robot Operating System. Im Abschnitt 3) finden sich Test-Ergebnisse wie gut die Algorithmen funktionieren. 
...
\end{figure}
	...
	Je unstimmiger die Daten des virtuellen Roboters sind, desto unwahrscheinlicher ist die Hypothese dass der reale sich dort befindet. Daher werden jene Partikel gel\"oscht. Im Bereich der wahrscheinlichen Hypothesen, werden neue Patikel/Hypothesen erzeugt.
...
{
	 Bild des Graphen einfügen wie Jonas
...
Geschwindigkeit langsam: 20   5cm Genauigkeit
				schnell: 90	  20cm 
...
Dieses Paper pr\"asentiert den Vergleich zwischen AMCL und Odometrie. Mit einem mobilen Frauenhofer-Roboter wird mittels gmapping eine 2D-Karte erstellt. Mit Hilfe der Karte kann sich der Roboter mit AMCL orientieren, in dem Sensordaten mit Wahrscheinlichkeitsvariablen der Karte verglichen werden. In verschiedenen Vorg\"angen f\"ahrt der Roboter selbst\"andig mittels Pfadverfolgung eine gegebene Punktewolke ab. Die angegebene Punkte werden pr\"azise abgefahren, je dichter der Pfad in Punkte ausgedr\"uckt ist, desto genauer wird der Pfad verfolgt.
...
\section{Danksagung}
Die Autoren danken Angel f\"ur die Unterst\"utzung und Hilfe bei verschiedenen Problemen mit dem Roboter und RobotOperatingSystem.
